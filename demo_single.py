...
for cls_name, label in [('awake',0),('sleepy',1)]:
    ...
    for fname in sorted(os.listdir(d))[:500]:
...
picks = []
picks += [s for s in samples if s[3]==0 and s[2]>0.99 and s[4]==0][-2:]
picks += [s for s in samples if s[3]==1 and s[2]>0.99 and s[4]==1][-2:]
picks += [s for s in samples if 0.5 < s[2] < 0.85][:2]
picks += [s for s in samples if s[3] != s[4]][:2]
